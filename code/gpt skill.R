# S1-only GPT skill table to output/S1/tables/gpt skill.tex
...
stats_exp_df <- df_filtered %>% group_by(group) %>%
...
long_exp_df <- stats_exp_df %>%
  pivot_longer(-group, names_to = c("variable","stat"), names_pattern = "^(.*)_(mean|sd)$") %>%
  mutate(variable = var_labels[variable])

wide_exp_stats <- long_exp_df %>% pivot_wider(names_from = stat, values_from = value) %>%
...
table_df <- wide_exp_stats %>% dplyr::select(variable, group, mean_sd) %>% tidyr::pivot_wider(names_from = group, values_from = mean_sd)
table_df[is.na(table_df)] <- "-"

grp1 <- "AI-Assisted high"; grp2 <- "AI-Assisted low_intermediate"; grp3 <- "AI-Led high"; grp4 <- "AI-Led low_intermediate"
...
diff_list <- lapply(varlist, function(v){
  ...
  tibble(variable_raw = v,
         diff12 = mean(a1,na.rm=TRUE)-mean(a2,na.rm=TRUE), p12 = t12,
         diff34 = mean(a3,na.rm=TRUE)-mean(a4,na.rm=TRUE), p34 = t34)
}) %>% bind_rows()

diff_list <- diff_list %>% mutate(variable = var_labels[variable_raw]) %>% dplyr::select(-variable_raw)

fmt_diff_p <- function(diff, p, minutes = FALSE){
  diff_out <- ifelse(minutes, sprintf("%.1f", diff), sprintf("%.3f", diff))
  p_out    <- ifelse(is.na(p), "NA", ifelse(p < 0.001, "<0.001", sprintf("%.3f", p)))
  paste0(diff_out, "<br>\\relax[", p_out, "]")
}

diff_df <- diff_list %>% mutate(
  diff12_fmt = mapply(fmt_diff_p, diff12, p12, minutes = variable %in% c("Minutes to reproduction","Minutes to first minor error","Minutes to first major error")),
  diff34_fmt = mapply(fmt_diff_p, diff34, p34, minutes = variable %in% c("Minutes to reproduction","Minutes to first minor error","Minutes to first major error"))
) %>% dplyr::select(variable, diff12_fmt, diff34_fmt) %>%
  rename(`AI-Assisted \\\\ High vs Low` = diff12_fmt, `AI-Led \\\\ High vs Low` = diff34_fmt)

final_df <- table_df %>% left_join(diff_df, by = "variable") %>% rename(Variable = variable)
...
table_body <- kable(latex_df, format   = "latex", booktabs = TRUE, escape   = FALSE,
                    align    = c("l","c","c","c","c","c","c"), hline_after = c(0), linesep  = "") %>%
...
cat(
  ...
  "}\n",
  "\\end{table}",
  file = "output/S1/tables/gpt skill.tex"
)
